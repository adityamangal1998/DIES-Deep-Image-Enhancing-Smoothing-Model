...
from tensorflow.keras.layers import (Input, Conv2D, MaxPooling2D, Dropout,
                                     UpSampling2D, BatchNormalization,add,Lambda)
...
def median_pool2d(x, k=3):
    median = tfa.image.median_filter2d(x,filter_shape=(k,k))
    return median
...
def build_model():
    ...
    decoded = Lambda(median_pool2d, arguments={'k': 7},output_shape=median_pool2d_output_shape)(decoded)
    decoded = Conv2D(3, (3, 3), activation='sigmoid', padding='same')(decoded)
...
model = build_model()
model.summary()
model_type = "unet_autoencoder_median_filter"
